...
# Headers matching incoming telemetry and AI evaluation
CSV_HEADERS = [
...
# ===============================
# SERIAL SETTINGS WITH AUTO-RECONNECT
# ===============================
BAUD_RATE = 115200
SERIAL_PORTS = ["/dev/ttyUSB1", "/dev/ttyUSB0", "/dev/ttyACM0", "/dev/ttyACM1"]
...
while True:
    try:
        ...
        if ser is None or not ser.is_open:
            print("Serial connection unavailable. Searching for Arduino port...")
            time.sleep(2)
            ser = connect_serial()
            continue
        ...
        if ser.in_waiting > 0:
            ...
            # Ignore empty lines or non-JSON debug text
            if not line or not (line.startswith('{') and line.endswith('}')):
                continue

            data = json.loads(line)

            # Parse raw sensor values from Arduino JSON stream
            ...
            # Run CO2 conversion & ML prediction
        ...
        print(f"\n[Hardware Alert] Serial Disconnected ({e}). Reconnecting...")
        if ser:
            try:
                ser.close()
            except Exception:
