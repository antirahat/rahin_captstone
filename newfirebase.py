...
# Reference to the single live node on Firebase
...
while True:
    try:
        if ser.in_waiting > 0:
            ...
            # Ignore empty lines or non-JSON debug text (e.g., Arduino startup banner)
            if not line or not (line.startswith('{') and line.endswith('}')):
                continue

            data = json.loads(line)

            # Safely parse raw sensor values from Arduino JSON stream
            ...
            # Run CO2 conversion & ML model prediction
            ...
            # Structure full payload for Firebase & Local CSV storage
            payload = {
                "timestamp": timestamp,
                "ph": round(ph, 2),
                "tds": round(tds, 1),
